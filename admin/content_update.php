...
$sql =$DB_con->prepare("UPDATE content SET cat_id='$article_type', text_title=:article_title, description=:short_article, full_text=:full_article, modified_date='$date', display='$show', sort=:sort, feature='$feature' WHERE id=:id_article");
...
// function moving images
	...
	if(!empty($thumbnail)){
				...
				// insert images
					...
					$sql_check=$DB_con->prepare("SELECT id_article FROM images where id_article=:id_article");
					$sql_check->execute(array(':id_article' => $id_article ));
					$count=$sql_check->rowCount();

					if($count!=0){

						$slqimg=$DB_con->prepare("UPDATE images SET img_title=:img_title, img_thumbnail='$thumbnail',
						...
						$slqimg=$DB_con->prepare("INSERT INTO images(id_article, img_title, img_thumbnail, img_original) value('$id_article', '$img_title','$thumbnail', '$original_image')");
						$slqimg->execute();

						}
			}else{

				  $slqimg=$DB_con->prepare("UPDATE images SET img_title=:img_title WHERE id_article=:id_article");
				  $slqimg->execute(array(':id_article' => $id_article, ':img_title' => $img_title));
				}
//Redirect to form page
echo "<script type='text/javascript'>window.location='../sr-admin.php?page=content&action=edit&msg=update&id_article=".$id_article."'</script>";
?>