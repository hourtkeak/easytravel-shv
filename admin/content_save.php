...
if($article_title!="" and $full_article!=""){
  ...
  $sql=$DB_con->prepare("INSERT INTO 
...
}else{echo "<script type='text/javascript'>window.location='../admin.php?op=article&msg=require&action=form'</script>";}
...
$sqlselect =$DB_con->prepare("SELECT id FROM content order by id DESC limit 1");
$sqlselect->execute();
...
$new_id=$resultid['id'];
	...
	if(!empty($thumbnail)){
			  ...
			  $slqimg=$DB_con->prepare("INSERT INTO images(id_article, img_title, img_thumbnail, img_original) value('$new_id', '$img_title','$thumbnail', '$original_image')");
			  $slqimg->execute();
			}


	//Redirect to form page
	echo "<script type='text/javascript'>window.location='../sr-admin.php?page=content&action=edit&msg=insert&id_article=".$new_id."'</script>";
?>
